...
def W_Matrix(data):
    ...
    for i in range(dim):
        ...
        for j in range(dim):
            y = data[j,:]
            createMatrix[i,j] = AffMatrix(x,y,0.1) #计算的高斯核函数放入创建的空矩阵中
...
if __name__ == '__main__':
    data = np.array(load_data("/Users/kinddle/课程任务/人工智能基础/project_1/spiral.txt"))
    m = data.shape[0]
    t = time.time_ns()
    label_predict = NCutSpectralClustering(data[0:m,1:],2)
